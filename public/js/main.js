...
async function renderCategories() {
  ...
  data.forEach((category) => {
    ...
    >
      <a href="category.html?category=${category.slug}" class="flex items-center justify-center p-2 h-full">
    ...
    <li>
      <a class="flex flex-col items-center relative hover:bg-gray-200 rounded-lg overflow-hidden" href="product-detail.html?id=${product.id}">
        <img
          ...
          alt="sale product 1"
        />
        <div class="py-3 lg:text-[20px] text-orange-600">
          <span>$</span>
    ...
    <li>
      <a class="flex flex-col items-center hover:bg-gray-200 rounded-lg overflow-hidden" href="product-detail.html?id=${product.id}">
        <img
          ...
          alt="sale product 1"
        />
        <div class="flex w-full items-center justify-between px-0 lg:px-3">
          <div class="text-[14px] lg:text-[18px] flex items-center gap-1">
            <span>${product.rating}</span>
            <i class="fa-solid fa-star text-orange-400 text-[10px] lg:text-[16px]"></i>
          </div>
          <div class="py-3 text-[10px] lg:text-[14px]">
            <span>$</span>
            <span>${((product.price * (100 - product.discountPercentage)) / 100).toFixed(2)}</span>
          </div>
        </div>
